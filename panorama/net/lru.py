...
from panorama.yoloembeddingnet.yoloembeddingnet import K_nn, k_nn
import time
...
class lrucache(object):
    ...
    def __init__(self, size, nnei, cache_type, callback=None, alg='brute', GPU_mode=True):
        ...
        self.nnei = nnei
        if GPU_mode:
            if (self.cache_type == 'defeatist'):
                self.neigh = K_nn(1, alg)
            if (self.cache_type == 'idealist'):
                self.neigh = K_nn(nnei, alg)
        else:
            #             if (self.cache_type=='defeatist'):
            #                 self.neigh=k_nn(1,alg)
            #             if (self.cache_type=='idealist'):
            ...
            if (self.cache_type == 'defeatist'):
                self.neigh = KNeighborsClassifier(n_neighbors=1, algorithm=alg)
            if (self.cache_type == 'idealist'):
                self.neigh = KNeighborsClassifier(
                    n_neighbors=nnei, algorithm=alg)
    ...
    def update_knn(self):
        self.y_res = [item[0] for item in self.items()]
        if self.cache_type == 'idealist':
            self.X_res = [item[1] for item in self.items()]
            self.neigh.fit(self.X_res, self.y_res)
        elif self.cache_type == 'defeatist':
            self.X_emb__res = [item[1][0] for item in self.items()]
            self.X_ddist__res = [item[1][1] for item in self.items()]
            self.neigh.fit(self.X_emb__res, self.y_res)
    ...
    def defeatist_search(self, emb, thresh):
        ret = []
        if len(self) == 0:
            return ret, 0
        begin = time.time()
        dists, inds = self.neigh.kneighbors([emb], n_neighbors=1)
        dur = time.time() - begin
        for i in xrange(len(dists)):
            for j in xrange(len(dists[i])):
                if(dists[i][j] <= self.X_ddist__res[inds[i][j]]):
                    self[self.y_res[inds[i][j]]]
                    ret.append((self.y_res[inds[i][j]], dists[i][j]))
        return ret, dur

    def idealist_search(self, emb, thresh):
        ret = []
        if len(self) == 0:
            return ret, 0
        begin = time.time()
        if (len(self.y_res) < self.nnei):
            dists, inds = self.neigh.kneighbors(
                [emb], n_neighbors=len(self.y_res))
        else:
            dists, inds = self.neigh.kneighbors([emb], n_neighbors=self.nnei)
        dur = time.time() - begin
        for i in xrange(len(dists)):
            for j in xrange(len(dists[i])):
                if(dists[i][j] <= thresh):
