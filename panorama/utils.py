...
def draw_boxes(image, boxes, labels, allowed_labels):
...
def draw_names(image, boxes, names_scores):
    image_h, image_w, _ = image.shape

    for box, name_score in zip(boxes, names_scores):
        xmin = int(box.xmin * image_w)
        ymin = int(box.ymin * image_h)
        xmax = int(box.xmax * image_w)
        ymax = int(box.ymax * image_h)
        ...
        cv2.rectangle(image, (xmin, ymin), (xmax, ymax), (0, 255, 0), 3)
        cv2.putText(image,
                    str(name_score[0][0][0]) + ' ' + str(name_score[0]
                                                         [0][1]) + ' ' + str(name_score[0][1]),
                    (xmin, ymin - 13),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    1e-3 * image_h,
                    (0, 255, 0), 2)
...
def crop_boxes(image, boxes, labels, allowed_labels):
    image_h, image_w, _ = image.shape
    cropped = []
    cropped_boxes = []
    for box in boxes:
...
def decode_netout_emb(netout, anchors, nb_class, obj_threshold=0.3, nms_threshold=0.3, obj_class=False):
...
#     netout=netout[...,d[...,4]> obj_threshold]
            ...
            for b in range(nb_box):
                ...
                if confidence > obj_threshold:
                    ...
                    box = BoundBox(x - w / 2, y - h / 2, x +
                                   w / 2, y + h / 2, confidence, classes)

                    boxes.append((box, (row, col, b)))

    # suppress non-maximal boxes
    for c in range(nb_class):
        sorted_indices = list(
            reversed(np.argsort([box[0].classes[c] * box[0].c for box in boxes])))

        for i in range(len(sorted_indices)):
            index_i = sorted_indices[i]

            if boxes[index_i][0].classes[c] == 0:
                continue
            else:
                for j in range(i + 1, len(sorted_indices)):
                    ...
                    if bbox_iou(boxes[index_i][0], boxes[index_j][0]) >= nms_threshold:
                        boxes[index_j][0].classes[c] = 0

    # remove the boxes which are less likely than a obj_threshold
    if obj_class:
        boxes = [box for box in boxes if box[0].c > obj_threshold]
    else:
        boxes = [box for box in boxes if box[0].get_score() > obj_threshold]
