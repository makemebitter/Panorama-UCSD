            ...
            if 'object' in elem.tag or 'part' in elem.tag:
                ...
                for attr in list(elem):
                    if 'name' in attr.tag:
                        ...
                        if onlyInLabels and obj['name'] not in labels:
                            continue
                        else:
                            img['object'] += [obj]
...
def parse_annotation_not_in_labels(ann_dir, img_dir, labels=[]):
    all_imgs = []
    seen_labels = {}
...
class BatchGenerator(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        if self.config['CASCADE_LEVEL'] == 1:
            return [x_batch, b_batch], y_batch
        elif self.config['CASCADE_LEVEL'] > 1:
            return [x_batch, b_batch], [y_batch for _ in range(self.config['CASCADE_LEVEL'])]
